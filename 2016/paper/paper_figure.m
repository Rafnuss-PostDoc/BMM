...
g_dens_mean = nanmea0n( g_denstrans_est.^(1/pow_a) ,3);
g_dens_sum = reshape( nansum( nansum( (g_denstrans_est).^(1/pow_a) .* area ,1) ,2), 1, g.nt);
...
for i_at=1:(length(g.time)-1)/4/24-1
    ...
    geoshow('landareas.shp', 'FaceColor', [0.5 0.7 0.5])
    geoshow('worldrivers.shp','Color', 'blue')
    surfm(g.lat2D,g.lon2D,log10(g_dens_a_avg(:,:,i_at)));
...
g.dens_sig(~g.mask_rain)=0;

it=1652+[1:4:4*14]-24*4;

figure('position',[0 0 1400 1400]); hold on;
for i_t=1:length(it)
    subplot(2,length(it)/2,i_t);
    worldmap([min([dc.lat]) max([dc.lat])], [min([dc.lon]) max([dc.lon])]); 
    plotm(coastlat, coastlon,'k');
    geoshow('landareas.shp', 'FaceColor', [0.5 0.7 0.5])
    geoshow('worldrivers.shp','Color', 'blue')
    surfm(g.lat2D,g.lon2D,log10(g.dens_est(:,:,it(i_t))));
    caxis([0 2])
end
