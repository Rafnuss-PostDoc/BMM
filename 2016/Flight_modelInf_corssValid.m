...
for i_d=1:numel(dc)
    ...
    nb_bird_2 = nb_bird;
    nb_bird_2(isnan(dc(i_d).ff) | isnan(dc(i_d).dd))=0;
    % remove speed above 25.
    nb_bird_2(dc(i_d).ff>25)=0;
    ...
    nb_bird_2(sum(nb_bird_2>0,2)<3,:)=0;

    w_MTR = nb_bird_2 ./ repmat(nansum(nb_bird_2,2),1,size(nb_bird_2,2));
    
    dc(i_d).vs = nanmean(w_MTR.* dc(i_d).v,2);
    dc(i_d).us = nanmean(w_MTR.* dc(i_d).u,2);
...
for i_d=1:numel(dc)
    id = dc(i_d).scoret'>=-1 & dc(i_d).scoret'<=1 & dc(i_d).denss>dens_lim & ~isnan(dc(i_d).vs);
    d=table();
    d.v=dc(i_d).vs(id);
    d.u=dc(i_d).us(id);
...
uv.trans.meandata = [mean(data.v) mean(data.u)];
uv.trans.stddata = [std(data.v) std(data.u)];

V = ( data.v-uv.trans.meandata(1)) / uv.trans.stddata(1);
U = ( data.u-uv.trans.meandata(2)) / uv.trans.stddata(2);

f = @(z,lambda) sign(-z) .* (lambertw(lambda.*z.^2)./lambda).^.5;
uv.trans.lambda(1) = fminsearch(@(parm) kstestmin(V,f,parm), 1);
uv.trans.lambda(2)= fminsearch(@(parm) kstestmin(U,f,parm), .7);

uv.trans.meanlambda = [mean(f(V,uv.trans.lambda(1))) mean(f(U,uv.trans.lambda(1)))];
uv.trans.stdlambda = [std(f(V,uv.trans.lambda(2))) std(f(U,uv.trans.lambda(2)))];

uv.vtrans = (f(V,uv.trans.lambda(1))-uv.trans.meanlambda(1) )/uv.trans.stdlambda(1);
uv.utrans = (f(U,uv.trans.lambda(2))-uv.trans.meanlambda(2) )/uv.trans.stdlambda(2);
...
% reverser transform
uv.trans.fU = @(data,uv) -(data.*uv.trans.stdlambda+uv.trans.meanlambda);
uv.tran.finv = @(data,uv) uv.trans.fU(data,uv).*exp(uv.trans.lambda/2.*uv.trans.fU(data,uv).^2) .* uv.trans.stddata + uv.trans.meandata;
...
%
uv.cov.d=[0 eps 100 200 300 600 1000 1500 2000];
uv.cov.t=[0 eps .02 .05 .07 .1 .2 .25  .35]; 
...
a = uv.tran.finv([uv.utrans_est uv.vtrans_est],uv);
uv.u_est=a(:,1);
uv.v_est=a(:,2);

a = uv.tran.finv([uv.utrans_est+norminv(.1).*uv.utrans_sig uv.vtrans_est+norminv(.1).*uv.vtrans_sig],uv);
uv.u_p10=a(:,1);
uv.v_p10=a(:,2);

a = uv.tran.finv([uv.utrans_est+norminv(.9).*uv.utrans_sig uv.vtrans_est+norminv(.9).*uv.vtrans_sig],uv);
uv.u_p90=a(:,1);
uv.v_p90=a(:,2);
