...
vlon = nan(g.nlat,g.nlon, g.nt);
vlat = nan(g.nlat,g.nlon, g.nt);
c = nan(g.nlat,g.nlon, g.nt);

latlonmask = repmat(g.latlonmask,1,1,g.nt);


nb_real = 6;%size(real_dens_ll,3);
...
for i=1:nb_real
    ...
    W(2:end-1,2:end-1,1:end-1,i) = c_tp1-c(2:end-1,2:end-1,2:end);
end
% c per unit volume
