...
sim=nan(numel(idt),size(real_dens_ll,3),g.nlm);

g_latlonmask_id = reshape((cumsum(g.latlonmask(:))),g.nlat,g.nlon);
...
%             uv(:,2,i) = reshape( nanmean(nanmean( guv.v_est(i_lat+(0:dd-1),i_lon+(0:dd-1),idt),1),2),[],1);
            ...
            id = g_latlonmask_id(i_lat+(0:dd-1),i_lon+(0:dd-1));
            sim(:,:,i) = nanmean(real_dens_ll(id(:),idt,:));
...
clear d
for i=1:size(real_dens_ll,1)
    d(i).sim = round(real_dens_ll(i,:,:),2);
    d(i).id = i;
end
str = jsonencode(d);
str=strrep(str(2:end-1),'},{','}\n{');
str=strrep(str,'id','_id');
fileID = fopen('BMM_web/exportGrid_sim_mongodb.json','w');
fprintf(fileID,str);
fclose(fileID);
