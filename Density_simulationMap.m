...
% save('data/Density_simulationMap_reassemble','real_res','real_ampli','real_dens')
...
worldmap([min([dc.lat]) max([dc.lat])], [min([dc.lon]) max([dc.lon])]); 
Frame(numel(mask_fullday)-1) = struct('cdata',[],'colormap',[]); 
hcoast=plotm(coastlat, coastlon,'k'); caxis([-4 6]); c=colorbar;c.Label.String = 'Bird density [Log bird/km^2]';
for i_t = 1:numel(mask_fullday)

    hsurf=surfm(g.lat2D,g.lon2D,log(real_dens(:,:,mask_fullday(i_t),i_real)));
